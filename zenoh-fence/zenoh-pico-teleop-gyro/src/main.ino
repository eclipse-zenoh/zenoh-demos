...
#define KEYEXPR_ROBOT "bot1/rt/cmd_vel"
...
#define Y_MAX_VALUE 2.0
#define Y_MIN_VALUE -2.0
...
void fence_red_callback(const z_sample_t *sample, void *arg)
{
    std::string val((const char*)sample->payload.start, sample->payload.len);
    int is_red_on = std::stoi(val, NULL, 10);
    if (is_red_on == 1) {
        is_blocked = true;
    } else {
        is_blocked = false;
...
void fence_green_callback(const z_sample_t *sample, void *arg)
{
    std::string val((const char*)sample->payload.start, sample->payload.len);
    int is_green_on = std::stoi(val, NULL, 10);
    if (is_green_on == 1) {
        is_blocked = false;
    } else {
        is_blocked = true;
...
void setup(void)
{
    ...
    Serial.print(KEYEXPR_ROBOT);
    Serial.println("...");
    pub = z_declare_publisher(z_session_loan(&s), z_keyexpr(KEYEXPR_ROBOT), NULL);
    ...
    z_owned_closure_sample_t callback_red = z_closure_sample(fence_red_callback, NULL, NULL);
    printf("Declaring Subscriber on '%s'...\n", KEYEXPR_RED_LIGHT);
    z_owned_subscriber_t sub_red = z_declare_subscriber(z_session_loan(&s), z_keyexpr(KEYEXPR_RED_LIGHT), z_closure_sample_move(&callback_red), NULL);
    if (!z_subscriber_check(&sub_red))
    {
    ...
    printf("Declaring Subscriber on '%s'...\n", KEYEXPR_GREEN_LIGHT);
    z_owned_subscriber_t sub_green = z_declare_subscriber(z_session_loan(&s), z_keyexpr(KEYEXPR_GREEN_LIGHT), z_closure_sample_move(&callback_green), NULL);
    if (!z_subscriber_check(&sub_green))
    {
        printf("Unable to declare subscriber.\n");
        exit(-1);
    }
